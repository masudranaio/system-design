...
- Compartment state changes must be **thread-safe at the per-compartment
  level** — two couriers should never both win a reserve on the same
  slot.
- Code lookup should be close to O(1) even as the number of issued
  tokens grows.
- The size-matching and assignment logic should be swappable without
  touching the deposit workflow — a new compartment size or a smarter
  assignment rule shouldn't mean rewriting `depositPackage()`.

<KeyStat
  value="30–150"
  label="compartments per station"
  detail="Per the network-level breakdown in the HLD lesson, a station's compartment mix runs roughly 40% small / 35% medium / 20% large / 5% oversized. At that size, a linear scan filtered by size and status (see Trade-offs below) stays fast enough that an indexed available-queue isn't worth the added bookkeeping."
/>
...
The class diagram is split into two, on purpose: the core domain
(occupancy and token bookkeeping) is one concern, and how the outside
world reaches that domain (orders, customers, the facade) is another.

<D2Diagram title="Amazon Locker — core locker domain" type="class" chart={`
direction: right
...
LockerLocation: {
  ...
  -tokensByCode: code-to-token map
  +findAvailableCompartment(size): Compartment
  +registerToken(token): void
  +openExpiredCompartments(): Compartment list
}
...
LockerLocation -> Compartment: aggregates (1 -> *)
LockerLocation -> AccessToken: indexes by code (1 -> *)
AccessToken -> Compartment: unlocks (1 -> 1)
AccessToken -> Package: maps to (1 -> 1)
Compartment -> Package: currently holds (1 -> 0..1)
`} />
...
mutates a compartment directly.
...
<D2Diagram title="Amazon Locker — order/customer integration context" type="class" chart={`
direction: right
...
Package: {shape: class}
LockerLocation: {shape: class}

Order -> Package: composes (1 -> *)
Customer -> Order: places (1 -> *)
Reservation -> Order: reserved for (1 -> 1)
Reservation -> LockerLocation: at (1 -> 1)
LockerService -> LockerLocation: coordinates
DeliveryAgent -> LockerService: uses
Customer -> LockerService: uses
`} />

`Package` and `LockerLocation` reappear here as thin references back to
the core-domain diagram above — this panel is about who reaches the
station and how, not a second copy of their fields. `Reservation` is
optional — it exists to support "reserve a slot before the courier
physically arrives," e.g. an e-commerce order that wants to guarantee
capacity at checkout time; a walk-up courier with no prior reservation
skips straight to `depositPackage()`.
...
<D2Diagram title="Compartment lifecycle" type="state" chart={`
direction: right
lifecycle_start: {shape: circle; width: 24; height: 24; label: ""}
lifecycle_start -> AVAILABLE
...
<D2Diagram title="Package lifecycle" type="state" chart={`
direction: right
lifecycle_start: {shape: circle; width: 24; height: 24; label: ""}
lifecycle_end: {shape: circle; width: 16; height: 16; label: ""}
lifecycle_start -> IN_TRANSIT
...
PICKED_UP -> lifecycle_end
RETURNED_TO_SENDER -> lifecycle_end
`} />

<D2Diagram title="AccessToken lifecycle" type="state" chart={`
direction: right
lifecycle_start: {shape: circle; width: 24; height: 24; label: ""}
lifecycle_end: {shape: circle; width: 16; height: 16; label: ""}
lifecycle_start -> ACTIVE: generateAccessToken()
ACTIVE -> CONSUMED: pickup() with the correct code
ACTIVE -> EXPIRED: expiresAt elapses
CONSUMED -> lifecycle_end
EXPIRED_note: "Row stays in the code -> token map.\\nDeleting it would make an expired code\\nlook identical to a code that never existed." {
  shape: text
  style.font-size: 14
}
EXPIRED -> EXPIRED_note: {style.stroke-dash: 3}
`} />
...
- **Strategy** — compartment assignment is swapped in behind a
  `CompartmentAssignmentStrategy` interface rather than hard-coded
  inside `depositPackage()`. This is a real interview branch point, not
  a single rule — three concrete implementations of the same interface:

<CompareTable
  title="CompartmentAssignmentStrategy — concrete strategies"
  rows={[
    {
      approach: "Exact-size-match",
      pros: [
        "The invariant 'a MEDIUM package is always in a MEDIUM compartment' is trivial to state and audit",
        "No cross-size accounting needed for capacity reporting or billing",
      ],
      cons: ["Utilization drops when one size class is full while a larger size sits empty"],
      chosenBecause: "Simplicity and a clean invariant matter more than utilization at single-station scale (30-150 compartments).",
    },
    {
      approach: "Best-fit fallback",
      pros: ["Improves utilization: a MEDIUM package can still be placed when only LARGE slots are free"],
      cons: ["Compartment size no longer implies package size, complicating any later reporting or billing that assumes they match"],
    },
    {
      approach: "Reserved sub-pool split",
      pros: ["Protects walk-up courier capacity from being starved by a surge of pre-reserved orders, or vice versa"],
      cons: ["Splits one pool into two, so each sub-pool alone has less slack and can hit 'no capacity' sooner"],
    },
  ]}
/>

Notification-channel selection (SMS vs push vs email) is a second,
independent application of the same pattern, even though the
notification internals themselves are out of scope here.
...
<D2Diagram title="Amazon Locker — single-station schema" type="er" chart={`
direction: right

LOCKER_LOCATIONS: {
  shape: sql_table
  location_id: string {constraint: primary_key}
  latitude: float
  longitude: float
  status: string
}
COMPARTMENTS: {
  shape: sql_table
  slot_id: string {constraint: primary_key}
  location_id: string {constraint: foreign_key}
  size: string
  status: string
}
PACKAGES: {
  shape: sql_table
  package_id: string {constraint: primary_key}
  compartment_id: string {constraint: foreign_key}
...
ACCESS_TOKENS: {
  ...
  package_id: string {constraint: foreign_key}
  expiration: datetime
  used_at: datetime
}
ACCESS_LOG: {
  shape: sql_table
  log_id: string {constraint: primary_key}
  actor: string
  action: string
  compartment_id: string {constraint: foreign_key}
  timestamp: datetime
...
LOCKER_LOCATIONS.location_id -> COMPARTMENTS.location_id: contains
COMPARTMENTS.slot_id -> PACKAGES.compartment_id: currently holds
CUSTOMERS.customer_id -> PACKAGES.customer_id: owns
PACKAGES.package_id -> ACCESS_TOKENS.package_id: has
COMPARTMENTS.slot_id -> ACCESS_LOG.compartment_id: logs
`} />
...
That cleanup-timing choice is about *when a row disappears*. A separate,
often-conflated question is *how the system notices a token has expired
in the first place* — three real mechanisms, not one:

<CompareTable
  title="Token-expiry detection mechanism"
  rows={[
    {
      approach: "Lazy check-on-read",
      pros: [
        "No background process to build, schedule, or monitor",
        "isExpired() is a pure comparison against expiresAt, evaluated only when something actually looks at the token",
      ],
      cons: ["A token that's expired but never looked up stays ACTIVE in the map until something reads it"],
      chosenBecause: "openExpiredCompartments() already does the enumeration staff need — a background pass would duplicate that work.",
    },
    {
      approach: "Active background sweep",
      pros: ["Catches expired tokens proactively, even if nothing ever reads them"],
      cons: ["Adds a scheduled job to build, run, and monitor for a single-station design that doesn't otherwise need one"],
    },
    {
      approach: "Hard delete on expiry",
      pros: ["Bounded storage — no accumulation of dead tokens"],
      cons: [
        "Loses the expired-vs-never-existed distinction the whole design depends on",
        "openExpiredCompartments() has nothing left to enumerate",
      ],
    },
  ]}
/>
...
Locking granularity has the same "two options isn't the whole story"
shape as token-expiry detection above — a third, lock-free option is a
real branch interviewers push on once the first two are on the table:

<CompareTable
  title="Concurrency-control mechanism for Compartment state changes"
  rows={[
    {
      approach: "Per-compartment pessimistic lock",
      pros: [
        "Two operations on two different compartments run fully in parallel",
        "Simple mental model: acquire the one lock that matters, nothing else blocks",
      ],
      cons: ["Still serializes the rare case of two operations racing for the exact same compartment"],
      chosenBecause: "Contention is naturally scoped to one compartment at a time, so lock scope should match it.",
    },
    {
      approach: "Single aggregate-wide lock",
      pros: ["One lock to reason about — no risk of acquiring the wrong one"],
      cons: ["Serializes every operation at the station, even ones touching unrelated compartments"],
    },
    {
      approach: "Optimistic concurrency (version column + CAS)",
      pros: ["No blocking at all under low contention"],
      cons: [
        "The caller must detect and retry a failed compare-and-swap",
        "Only pays off when same-compartment contention is rare — otherwise retries pile up",
      ],
    },
  ]}
/>
...
- **Size-matching strictness: strict exact-match (chosen here) vs.
  ...
  compartment size and package size are the same thing. (This is the
  same trade-off as the `CompartmentAssignmentStrategy` comparison
  above, restated as a chosen-vs-alternative pair.)
...
collects. Three focused diagrams, one per concern, instead of one
diagram carrying the whole trace.

<D2Diagram title="Deposit" type="sequence" chart={`
shape: sequence_diagram

Courier: {style.fill: "#3b6fd6"; style.font-color: "#ffffff"}
LockerService: {style.fill: "#0e7c86"; style.font-color: "#ffffff"}
LockerLocation: {style.fill: "#0e7c86"; style.font-color: "#ffffff"}
CompartmentC14: "Compartment C-14 (medium)"
AccessToken
NotificationService: {style.fill: "#0e7c86"; style.font-color: "#ffffff"}
Customer: {style.fill: "#3b6fd6"; style.font-color: "#ffffff"}
...
CompartmentC14 -> CompartmentC14: note -- AVAILABLE -> RESERVED
...
CompartmentC14 -> CompartmentC14: note -- RESERVED -> OCCUPIED
...
<D2Diagram title="Pickup: wrong code, then correct code" type="sequence" chart={`
shape: sequence_diagram
...
AccessToken -> AccessToken: note -- ACTIVE -> CONSUMED
LockerService -> CompartmentC14: release()
CompartmentC14 -> CompartmentC14: note -- OCCUPIED -> AVAILABLE
`} />
...
<D2Diagram title="Unclaimed package: expiry and staff sweep" type="sequence" chart={`
shape: sequence_diagram

Staff: {style.fill: "#3b6fd6"; style.font-color: "#ffffff"}
LockerService: {style.fill: "#0e7c86"; style.font-color: "#ffffff"}
AccessToken: "AccessToken (code 771102)"
CompartmentC09: "Compartment C-09 (medium)"

AccessToken -> AccessToken: note -- day 3, expiresAt elapses
AccessToken -> AccessToken: note -- ACTIVE -> EXPIRED, row stays mapped

Staff -> LockerService: openExpiredCompartments()
LockerService -> AccessToken: find tokens where status = EXPIRED
AccessToken -> LockerService: "found: C-09, code 771102"
...
CompartmentC09 -> CompartmentC09: note -- OCCUPIED -> AVAILABLE
`} />
...
returns can't starve incoming deliveries (or vice versa)? That's exactly
the reserved-sub-pool-split branch of the `CompartmentAssignmentStrategy`
comparison above, applied to a real scenario — a second, separate
strategy (a `CompartmentPoolStrategy` alongside the existing
size-matching one), not a variant of the size-matching rule.
